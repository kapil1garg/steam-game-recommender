...
f1.0.plot <- ggplot(raw.data, aes(x = Algorithm, y = F1_0, fill = Algorithm)) +
  ...
  ggtitle("Boxplot of F1 Measure (TP = 0) for each Algorithm") +
  guides(fill = guide_legend(order = 1, title = NULL))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.text.y = element_text(vjust = 0.25),
...
f1.1.plot <- ggplot(raw.data, aes(x = Algorithm, y = F1_1, fill = Algorithm)) +
  ...
  ggtitle("Boxplot of F1 Measure (TP = 1) for each Algorithm") +
  guides(fill = guide_legend(order = 1, title = NULL))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.text.y = element_text(vjust = 0.25),
        panel.grid.minor = element_line(color = "gray", size = 0.3), panel.grid.major = element_line(color = "gray", size = 0.3), panel.margin = unit(x = 0.12, "inches"),
...
accuracy.plot <- ggplot(raw.data, aes(x = Algorithm, y = Accuracy, fill = Algorithm)) +
  ...
  xlab("Algorithm") +
  ylab("Accuracy") +
  ggtitle("Boxplot of Accuracy for each Algorithm") +
  guides(fill = guide_legend(order = 1, title = NULL))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.text.y = element_text(vjust = 0.25),
        panel.grid.minor = element_line(color = "gray", size = 0.3), panel.grid.major = element_line(color = "gray", size = 0.3), panel.margin = unit(x = 0.12, "inches"),
...
ggplot(raw.data, aes(x = NPlayers, y = F1_1, color = Algorithm)) +
  ...
  ylab("F1 Measure (TP = 1)") +
  ggtitle("Scatterplot of F1 Measure (TP = 1) for each Algorithm with Regression Line") +
