...
class KNNClassifier:
    ...
    def classifyGames(self, k):
        ...
        for g in self.game_list:
            g_index = len(self.header_row) - self.header_row.index(g) - 1
            true_labels = []
            classification = []

            print "Testing game " + str(g) + " with " + str(k) + " nearest neighbors"

            # for i in range(0, 10): # FOR TESTING ONLY!!!
            for i in range(len(self.test_set)):
                t = self.test_set[i]
                true_labels.append(nthBit(t, g_index))
    ...
    def hammingDistance(self, instance1, instance2, ignore_index):
        ...
        return bin(instance1 ^ instance2).count("1") - (nthBit(instance1, ignore_index) != nthBit(instance2, ignore_index))
